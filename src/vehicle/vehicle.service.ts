...
async update(vehicleId: string, dto: UpdateVehicleDto) {
  ...
  if (dto.carImageKey && vehicle.carImageKey && dto.carImageKey !== vehicle.carImageKey) {
  ...
  try {
    ...
    if (err?.code === 11000) throw new ConflictException('chassisNumber already exists');
    throw err;
  }
}
...
async findAll(query: VehicleQueryDto) {
  ...
  if (query.variant) filter.variant = new RegExp(`^${escapeRegex(query.variant)}$`, 'i');
  if (query.registrationNumber)
    filter.registrationNumber = new RegExp(escapeRegex(query.registrationNumber), 'i');
  ...
  if (query.yearOfManufacture) filter.yearOfManufacture = query.yearOfManufacture;
